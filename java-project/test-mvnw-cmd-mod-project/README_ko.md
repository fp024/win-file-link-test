...
Maven Wrapper 3.3.4에서 mvnw.cmd를 미리 변경해서 실행시켜본 프로젝트
...
- Jenkins Freestyle 프로젝트의 **Execute Windows batch command** 스텝안에서,  실행하려는 mvnw 명령을 x86환경 cmd.exe로서 사용되도록 빌드 스텝의 커맨드에 아래와 같이 입력합니다.

  ```bat
  C:\Windows\SysWOW64\cmd.exe /c mvnw clean test
  ```

- Jenkins 빌드가 `SUCCESS`로 처리되는지 여부를 검증합니다.



## 기대하는 동작

- 테스트 코드는 항상 성공하는 코드이며, Jenkins Job은 **성공(SUCCESS)** 처리되어야 합니다.
